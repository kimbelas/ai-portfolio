...
An agent built from its atom (LLM + tools + loop), then on **LangGraph.js**: tool calling, multi-step planning, **human-in-the-loop approval** for risky actions, **guardrails** (input validation, prompt-injection defense, allow-list, timeouts, step budget), **observability** (per-node token/cost/latency trace), and **checkpoints** (pause/resume + cross-turn memory). 9-rung build (A1–A9), all runnable — **A9 is an eval harness** (`npm run eval`): tool-selection accuracy 100%, task success 100%, and a check that the risky tool pauses for human approval. **Deployable as an HTTP service** (`npm run web` / `render.yaml`) with **human-in-the-loop approval over HTTP** — verified end-to-end (approve sends the email, deny cancels it).
