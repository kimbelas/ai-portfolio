...
Retrieval-Augmented Generation: ingest documents → **hybrid retrieval** (semantic + keyword, Reciprocal Rank Fusion) → **grounded, cited answers** with an "I don't know" refusal. Ships with an **eval harness** (retrieval recall@k + LLM-as-judge + refusal accuracy) and a **production layer** (streaming, relevance-floor guardrail, cost meter). 8-rung build from a single LLM call up. See its [walkthrough](./ai-knowledge-assistant/docs/WALKTHROUGH.md), [ADRs](./ai-knowledge-assistant/docs/adr), and the [benchmark writeup](./ai-knowledge-assistant/docs/blog/measuring-rag-quality.md) — how the numbers were produced (recall **93% → 100%** with a reranker; **100%** refusal on out-of-doc questions).
